                ...
                <div class="row">
                    ...
                    <div class="col-sm-12">
                        <h2 class="text-center">
                            <span>4-/</span>Faire des requetes avec <code>query()</code>  et afficher plusieurs résultats
                        </h2>
                        <?php
                        $requete = $pdoENT->query("SELECT * FROM employes ORDER BY prenom");
                        $nbr_employes = $requete->rowCount();
                        // cette methode rowCount() permet de compter le nombre d'enregistrements retournés par la requête
                        echo"<p> Il y a " .$nbr_employes.
                        " employés dans notre BDD.</p>";
                        // comme nous avons plusieurs résultats dans $requete ; noius devons faire une boucle pour les parcourir 
                        // fetch() va  chercher la ligne suivante du jeu de resultat à chaque tour dse boucle et le transforme en 
                        // ... objet . La boucle while permet de faire avancer le curseur dans l'objet. Quand il arrive à la fin , fetch()retourne FALSE et la boucle s'arrete
                        echo"<ul>";
                        while($ligne = $requete->fetch(PDO::FETCH_ASSOC)){
                            echo"<li>" .$ligne['prenom']."".$ligne['nom']."-".$ligne['sexe']."-".$ligne['service']."-".$ligne['date_embauche']."-".$ligne['salaire']."€</li>";
                        }
                        echo "</ul>";
                        // exo afficheur la liste des différents services dans une ul en mettant un service par li 
                        $requete = $pdoENT ->query("SELECT DISTINCT(service)FROM employes ORDER BY service ");
                        $nbr_services = $requete->rowCount();

                        echo"<di class=\"bg-info rounded w-50 text-white mt-4 mx-auto\">";
                        echo"<p class=\"p-2text-white\"> Il y a " .$nbr_services. " services dans l'entreprise : </p>";
                        echo "</div>";
